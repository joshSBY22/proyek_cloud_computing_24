...
const {
	findNearbyTickets,
	haversineDistance,
} = require("../helper/haversineDistance");
...
async function create(req, res) {
	...
	if (isNaN(latitude) || isNaN(longitude)) {
		return res.status(400).send({
			message:
				"Latitude and Longitude must be a number and separated by comma ','",
		});
	...
	if (imageFile) {
		try {
			...
			const newTicketId = `TIC${String(
				(await db.collection("tickets").count().get()).data().count + 1
			).padStart(6, "0")}`;
			...
			const newTicket = await db
				...
				.set({
					...
					type: ticket_type,
					start_datetime:
						ticket_type === "event" ? new Date(req.body.start_datetime) : null,
					end_datetime:
						ticket_type === "event" ? new Date(req.body.end_datetime) : null,
			...
			return res.status(200).send({
				...
				data: {
					...
					start_datetime:
						ticket_type === "event" ? req.body.start_datetime : null,
			...
			return res
				.status(500)
				.send({ message: `Failed to create ticket`, error: error.message });
		}
	} else {
...
async function getTicketsNearby(req, res) {
	const { latitude_position, longitude_position, radius_in_kilometre } =
		req.query;
	...
	try {
		...
		const nearbyTickets = findNearbyTickets(
			tickets,
			latitude,
			longitude,
			radius_in_kilometre
		...
		if (nearbyTickets.length > 0) {
			nearbyTickets.forEach((ticket) => {
				...
				ticket.distance = `${haversineDistance(
					latitude,
					longitude,
					ticket.latitude_position,
					ticket.longitude_position
...
async function getById(req, res) {
	...
	try {
		...
		// Check if the ticket and username is exist in logs collection
		const log = await db.collection("logs").where("ticket_id", "==", id_ticket).where("username", "==", userUsername).get();
		...
		const newLog = `LOG${String(
			(await db.collection("logs").count().get()).data().count + 1
		).padStart(6, "0")}`;
		...
		await db
			...
			.set({
				...
				title: getTicketData.title,
				price: parseInt(getTicketData.price),
				description: getTicketData.description,
			});
...
async function update(req, res) {
	...
	try {
		...
		if (ticket.data().creator !== userUsername) {
			return res
				.status(401)
				.send({ message: "Only can update ticket created by you" });
		}
		...
		const latitude_longitude = req.body.ticket_latitude_longitude
			? req.body.ticket_latitude_longitude.split(",")
			: [];
		const latitude =
			latitude_longitude.length > 0
				? String(latitude_longitude[0]).trim()
				: null;
		const longitude =
			latitude_longitude.length > 0
				? String(latitude_longitude[1]).trim()
				: null;
		...
		const ticketOldValue = (
			await db.collection("tickets").doc(id_ticket).get()
		).data();
		...
		const updateTicket = await db
			...
			.update({
				...
				street_location:
					req.body.ticket_location ?? ticketOldValue.street_location,
...
async function remove(req, res) {
	...
	try {
		...
		return res
			.status(200)
			.send({ message: `Ticket ${id_ticket} removed successfully!` });
	} catch (error) {
		return res
			.status(500)
			.send({ message: "Internal Server Error", error: error.message });
